...
public interface Repository <T extends Object> {
	...
	public default List<T> findAll(){
		...
		DatabaseManager.getInstance().getCollection(getObjectOfT().getClass().getSimpleName()).find().forEach(convertBlock);
		return objects;
	}
	...
	public default T find(String field, String id) {
		try {
			if(getObjectOfT() != null) {
				Document document = DatabaseManager.getInstance().getCollection(getObjectOfT().getClass().getSimpleName()).find(Filters.eq(field, id)).first();
				if(document != null) {
					try {
						DocumentAdapter<T> documentAdapter = DocumentAdapter.getInstance();
	...
	public default void update(String field, String id, T newObject) {
		...
		if(object != null) {
			Document document = DatabaseManager.getInstance().getCollection(getObjectOfT().getClass().getSimpleName()).find(Filters.eq(field, id)).first();
			DatabaseManager.getInstance().getCollection(getObjectOfT().getClass().getSimpleName()).replaceOne(Filters.eq("_id", new ObjectId(document.get("_id").toString())), DocumentAdapter.getInstance().getDocument(newObject));
		} else {
			save(newObject);
		}
	...
	public default void save(T object) {
		...
		DatabaseManager.getInstance().getCollection(getObjectOfT().getClass().getSimpleName()).insertOne(DocumentAdapter.getInstance().getDocument(object));
	}
